...
$most_searchedd_database_result = mysql_query("SELECT totalQueries.query from totalQueries INNER JOIN mostSearchedRareDiseases ON mostSearchedRareDiseases.rareDiseaseId = totalQueries.id ORDER BY mostSearchedRareDiseases.searchNumber DESC");
...
while ($most_searched_row = mysql_fetch_array($most_searchedd_database_result)) {
	...
	$most_searched_element = '<div class="mostSearchedResult roundedCorner"><a href=searchResult.php?disease_chosen=' . $most_searched_row_html_encoded . '>' . $most_searched_row{'query'} . '</a> </div>';
